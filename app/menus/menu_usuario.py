...
def fazer_simulacao(id_usuario):
    cabecalho("SIMULAÇÃO DE ECONOMIA", "⚡")
    ...
    try:
        ...
        if consumo_mensal < 0 or valor_fatura < 0:
            print("\n❌ Erro: consumo e valor da fatura não podem ser negativos.")
            return
        ...
        cabecalho("COMPARAÇÃO DE SOLUÇÕES", "💡")
...
def deletar_simulacao(id_usuario):
    ...
    try:
        ...
        if sucesso:
            ...
            print("\n❌ Simulação não encontrada para este usuário.")
...
def visualizar_solucoes_disponiveis(id_usuario):
    ...
    cabecalho("SOLUÇÕES DISPONÍVEIS", "💡")
    ...
    try:
        ...
        cabecalho("PARCEIROS COMPATÍVEIS", "🏢")
        ...
        if demonstrar_interesse.strip().lower() != "s":
            print("\nNenhum interesse foi registrado.")
            return
        ...
        if id_parceiro not in ids_parceiros_disponiveis:
            print("\n❌ Parceiro inválido. Escolha apenas um parceiro listado.")
            return
        ...
        if id_lead is None:
            print("\n⚠️ Já existe um interesse ativo para esse parceiro e essa solução.")
            return

        print(f"\n✅ Interesse registrado com sucesso. Código do lead: {id_lead}")

    except ValueError:
        print("\n❌ Erro: informe um ID válido.")

    except Exception as erro:
        print("\n❌ Erro ao visualizar soluções:")
        print(erro)
...
def cancelar_interesse(id_usuario):
    cabecalho("CANCELAR INTERESSE", "⚠️")
    ...
    try:
        ...
        if sucesso:
            ...
            print("\n❌ Lead não encontrado para este usuário.")

    except ValueError:
        print("\n❌ Erro: informe um ID válido.")

    except Exception as erro:
        print("\n❌ Erro ao cancelar interesse:")
        print(erro)
